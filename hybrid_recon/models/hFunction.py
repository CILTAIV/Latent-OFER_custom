import torch
from util.NonparametricShiftForHybrid import NonparametricShift
...
class HybridFunction(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, input,input_flip, mask, shift_sz, stride, triple_w, flag, nonmask_point_idx,mask_point_idx ,flatten_offsets, sp_x, sp_y):
        ...
        for idx in range(ctx.bz):
            ...
            output_flip = input_flip.narrow(0,idx,1)
            Nonparm = NonparametricShift()

            
            conv_enc_total, conv_enc, conv_new_dec,_,known_patch, unknown_patch, total_patch = Nonparm.buildAutoencoder(inpatch.squeeze(), False, False, nonmask_point_idx,mask_point_idx,  shift_sz, stride)
            
            output_var = Variable(output)
            output_flip_var = Variable(output_flip)
            
            tmp1 = conv_enc(output_var) 
            tmp2 = conv_enc_total(output_flip_var) 

            maxcoor = MaxCoord()
            kbar, ind, vmax = maxcoor.update_output(tmp1.data, sp_x, sp_y)
            _, _, vmax_flip = maxcoor.update_output(tmp2.data, sp_x, sp_y)
            ...
            refer_patch = total_patch.clone()
            refer_patch = refer_patch.zero_()
            ...
            for i in range(kbar_h):
                for j in range(kbar_w):
                    ...
                    if i==0:
                        if j==0:
                           refer_patch[indx] = (total_patch[indx]+total_patch[indx+1]+total_patch[indx+kbar_h]+total_patch[indx+kbar_h+1])/4
                        elif j==31:
                           refer_patch[indx] = (total_patch[indx]+total_patch[indx-1]+total_patch[indx+kbar_h]+total_patch[indx+kbar_h-1])/4
                        else:
                           refer_patch[indx] = (total_patch[indx]+total_patch[indx-1]+total_patch[indx+1]+total_patch[indx+kbar_h]+total_patch[indx+kbar_h-1]+total_patch[indx+kbar_h+1])/6
                    elif i==31:
                        if j==0:
                           refer_patch[indx] = (total_patch[indx]+total_patch[indx+1]+total_patch[indx-kbar_h]+total_patch[indx-kbar_h+1])/4
                        elif j==31:
                        ...
                        if j==0:
                            refer_patch[indx] = (total_patch[indx]+total_patch[indx+1]+total_patch[indx-kbar_h]+total_patch[indx-kbar_h+1]+total_patch[indx+kbar_h]+total_patch[indx+kbar_h+1])/6
                        elif j==31:
                            refer_patch[indx] = (total_patch[indx]+total_patch[indx-1]+total_patch[indx-kbar_h]+total_patch[indx-kbar_h-1]+total_patch[indx+kbar_h]+total_patch[indx+kbar_h-1])/6
                        else:
                            refer_patch[indx] = (total_patch[indx]+total_patch[indx-1]+total_patch[indx+1]+total_patch[indx-kbar_h-1]+total_patch[indx-kbar_h]+total_patch[indx-kbar_h+1]+total_patch[indx+kbar_h]+total_patch[indx+kbar_h-1]+total_patch[indx+kbar_h+1])/9
            ...
            for i in range(kbar_h):
                for j in range(kbar_w):
                    ...
                    if(check.sum()>=1):
                        ...
                        if ind_laten==0: 
                            out_new[ind_laten]=0.5*known_region+0.5*refer_patch[indx] 
                            ...
                            at_final_new = at_value_m / (at_value_m + vamx_mask[ind_laten]+vmax_flip[ind_laten])
                            
                            at_flip_inf = vmax_flip[ind_laten]/ (at_value_m + vamx_mask[ind_laten]+vmax_flip[ind_laten])
                            at_final_ori = vamx_mask[ind_laten] / (at_value_m + vamx_mask[ind_laten]+vmax_flip[ind_laten])
                            out_new[ind_laten] = (at_final_new) * out_new[ind_laten - 1] + (at_final_ori) * known_region+(at_flip_inf) * refer_patch[indx] 
